...
│ │
│ ├── train_dqn_seeds_best.py          ← seed 학습 + best seed 재학습 + test
│ ├── train_ppo_continuous.py          ← seed 학습 + best seed 재학습 + test
│ ├── train_ppo_continuous_hparam.py   ← PPO 하이퍼파라미터 실험 (lr/dd_penalty)
...
## PPO Hyperparameter 실험

PPO는 연속 행동 정책을 사용하기 때문에  
**learning rate**와 **drawdown penalty(dd_penalty)** 변화에 민감하게 반응한다.  
이를 확인하기 위해 아래 세 가지 설정을 비교했다.
...
### 해석
- learning rate 증가(lr3e-4)는 **성능 불안정**  
- dd_penalty 완화(dd=0.1)는 **리스크 증가(MDD 상승)**  
- 기본 설정(base_lr1e-4_dd0.3)이 가장 안정적  
- 전반적으로 baseline(EW/BH)을 안정적으로 초과하지는 못함  
- 작은 자산 수(3개) + 금융 시계열 노이즈 환경에서는  
  **reward/state 구조 → 성능 영향 > hyperparameter 영향**  
- PPO는 학습률과 dd_penalty 조정으로 정책 안정성 차이를 보이나  
  강한 초과성과는 나타나지 않음
...
## 학습된 모델 다운로드 (Trained Model Download)

본 프로젝트에서 학습한 모든 강화학습 모델(DQN / PPO)은  
GitHub Releases에 업로드되어 있으며, 아래 링크에서 다운로드할 수 있습니다.

🔗 **[Download Trained Models](https://github.com/ohsy1229/RL_project/releases/latest)**

### 제공되는 모델 목록
- `dqn_best.zip` — Best DQN 모델  
- `dqn_seed_0.zip`, `dqn_seed_42.zip`, `dqn_seed_2024.zip`  
- `ppo_cont_best.zip` — Best PPO(Continuous) 모델  

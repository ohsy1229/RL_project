...
def run_policy_on_env_nav(model, env: PortfolioEnvDiscrete) -> pd.Series:
    """
    정책 실행 후 NAV 시계열을 반환하는 함수이다.
    env.step() 내부에서 NAV는 'net_ret(= 수익률 - 거래비용)' 기준으로 업데이트되므로,
    여기서 얻는 NAV는 비용 반영 후 포트폴리오 가치 궤적이다.
    """
    obs, info = env.reset()
...
def main():
    ...
    if nav_paths:
        ...
        plt.title("Validation NAV Paths by Seed")
        plt.xlabel("Time (validation period)")
        plt.ylabel("NAV")
        plt.legend()
        plt.grid(True, alpha=0.3)

        fig_path = os.path.join(figs_dir, "nav_valid_by_seed.png")
        plt.savefig(fig_path, dpi=200, bbox_inches="tight")
        plt.close()
        print(f"Saved figure: {fig_path}")

    # 2) Test 구간 RL vs Baseline NAV 비교 (best model 기준)
    best_model_path = os.path.join(models_dir, "dqn_best.zip")
    if os.path.exists(best_model_path):
        model_best = DQN.load(best_model_path)
        ...
        plt.figure(figsize=(10, 5))
        plt.plot(x, nav_rl.values[:min_len], label="RL (DQN)")
        plt.plot(x, nav_ew.values[:min_len], label="Equal Weight")
        plt.plot(x, nav_bh.values[:min_len], label="Buy & Hold")
        plt.title("Test NAV Comparison: RL vs Baselines")
        plt.xlabel("Time (test period)")
        plt.ylabel("NAV")
        plt.legend()
        plt.grid(True, alpha=0.3)

        fig_path = os.path.join(figs_dir, "nav_test_comparison.png")
        plt.savefig(fig_path, dpi=200, bbox_inches="tight")
        plt.close()
        print(f"Saved figure: {fig_path}")
    else:
        print(f"Best model not found at {best_model_path}, skip test NAV plot.")
